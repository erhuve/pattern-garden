...
with sync_playwright() as p:
    ...
    for size in [(1280,800),(390,700)]:
        ...
        for fixture in fixtures:
            ...
            if slug == 'front-door-bench':
                garden_solution = saved(slug)
                assert not any(p.get('kind') == 'path' and p.get('y') == 8 for p in garden_solution)
                expect(page.get_by_role('button', name=re.compile('^Path stone,'))).to_contain_text('4/12')
                page.get_by_role('button', name=re.compile('^Path stone,')).click()
                tap_cell(4, 8)
                expect(page.get_by_role('button', name=re.compile('^Path stone,'))).to_contain_text('5/12')
                expect(page.locator('.pg-score-value span')).to_have_text('100')
                legacy_solution = saved(slug)
                page.reload()
                expect(page.locator('.pg-score-value span')).to_have_text('100')
                assert saved(slug) == legacy_solution
                page.get_by_role('button', name='Remove', exact=True).click()
                tap_cell(4, 8)
                expect(page.get_by_role('button', name=re.compile('^Path stone,'))).to_contain_text('4/12')
                expect(page.locator('.pg-score-value span')).to_have_text('100')
                assert saved(slug) == garden_solution
                page.reload()
                expect(page.locator('.pg-score-value span')).to_have_text('100')
                assert saved(slug) == garden_solution
                page.get_by_role('button', name='Remove', exact=True).click()
                ...
                expect(page.locator('.pg-adaptation')).to_contain_text('no stone is needed on it')
